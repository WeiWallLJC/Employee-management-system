	...
	public update(JFrame w) {
		...
		confirm.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(text1.isEmpty())
					JOptionPane.showMessageDialog(null, "要修改的员工信息不能为空！", "提示", 2);
				else if(text2=="姓名"&&!stfdbop.selectName(text1))
					JOptionPane.showMessageDialog(null, "员工信息不存在！", "提示", 2);
				else if(text2=="工号"&&!stfdbop.selectnum(text1))
					JOptionPane.showMessageDialog(null, "员工信息不存在！", "提示", 2);
				else
				{
					f.dispose();
					Modify modify=new Modify(text1,w,text2);
				}
			}
		});
